...
class Experiment:
    def __init__(self, encoder="QGNN", decoder="TuckER", num_iterations=4000, batch_size=1024, learning_rate=0.01, 
                 label_smoothing=0.1, hidden_dim=128, emb_dim=128, num_layers=1, eval_step=100,
    ...
    def train_and_eval(self):
        ...
        if self.encoder.lower() == "qgnn":
            print("Training with the QGNN encoder")
            if self.decoder.lower() == "tucker":
                model = QTuckER(self.emb_dim, self.hid_dim, adj, len(self.entity_idxs), len(self.relation_idxs), self.num_layers, **self.kwargs).to(device)
                print("and the TuckER decoder")
            else:
                raise ValueError('This decoder does not exist')
        elif self.encoder.lower() == "ognn":
            print("Training with the OGNN encoder")
            if self.decoder.lower() == "tucker":
                model = OTuckER(self.emb_dim, self.hid_dim, adj, len(self.entity_idxs), len(self.relation_idxs), self.num_layers, **self.kwargs).to(device)
                print("and the TuckER decoder")
            else: 
              raise ValueError('This decoder does not exist')
        else:
            raise ValueError('This encoder does not exist')
        ...
        for it in range(1, self.num_iterations + 1):
            ...
            if not it % self.eval_step:
                ...
                print("Best valid epoch is", best_epoch, " --> Final test results: ", final_test_h10, final_test_h3, final_test_h1, final_test_mr, final_test_mrr)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", type=str, default="WN18RR", nargs="?", help="WN18RR, FB15k237")
    ...
    parser.add_argument("--decoder", type=str, default="TuckER", nargs="?")
    ...
    d = Data(data_dir=data_dir, reverse=True) #default Data(data_dir=data_dir)
    experiment = Experiment(encoder=args.encoder, decoder=args.decoder, num_iterations=args.num_iterations, 
                 batch_size=args.batch_size, learning_rate=args.lr, hidden_dim=args.hidden_dim, emb_dim=args.emb_dim, 
                 num_layers=args.num_layers, eval_step=args.eval_step, input_dropout=args.input_dropout, 
                 hidden_dropout1=args.hidden_dropout1, hidden_dropout2=args.hidden_dropout2)
